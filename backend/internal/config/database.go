	...
	operations := []struct {
		...
		{"Auto migrate (dev only)", func() error {
			if !util.IsProduction() {
				return db.AutoMigrate(&model.User{}, &model.RefreshToken{}, &model.Problem{}, &model.Example{}, &model.TestCase{}, &model.IOSchema{}, &model.Match{}, &model.ActiveMatchParticipant{})
			}
			return nil
		}},
